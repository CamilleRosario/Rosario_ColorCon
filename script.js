...
let calcRating = (Contrast_Value) => {
    if (Contrast_Value > 12) {
        ...
        return "Very Good";
    } else if (Contrast_Value > 5) {
        result.style.backgroundColor = "#f7d658";
        return "Good";
    } else if (Contrast_Value > 3) {
...
let contrastChecker = (button) => {
let textColorValue = textColor.value;
let textColorRGBArray = HexColorToRGB(textColorValue);

let bgColorValue = bgColor.value;
let bgColorRGBArray = HexColorToRGB(bgColorValue);
